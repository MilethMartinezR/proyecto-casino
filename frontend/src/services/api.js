...
async function request(url, options = {}) {
  ...
  if (!res.ok) {
    ...
    try {
      ...
      else if (Array.isArray(body.detail)) msg = body.detail.map(e => e.msg || e.message).join(', ')
...
export const authService = {
  register: (email, password, role = 'PLAYER') =>
...
// Adapts backend game response to the shape GamePage expects:
// manoJugador/manoDealer come as flat arrays; frontend expects { cartas, puntos }
function adaptGame(res) {
  if (!res) return res
  return {
    ...res,
    manoJugador: {
      cartas: (res.manoJugador || []).map(c => ({ ...c, oculta: false })),
      puntos: res.puntosJugador ?? 0,
    },
    manoDealer: {
      cartas: (res.manoDealer || []).map(c =>
        c.valor === '?' ? { ...c, oculta: true } : { ...c, oculta: false }
      ),
      puntos: res.puntosDealer ?? 0,
    },
    pago: res.pago ?? 0,
  }
}
