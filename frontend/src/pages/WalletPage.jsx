            ...
            <form onSubmit={handleWithdraw} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  style={cuenta.length > 0 && cuenta.length < 3
                    ? { borderColor: 'var(--danger, #e74c3c)' }
                    : cuenta.length >= 3
                      ? { borderColor: 'var(--success, #2ecc71)' }
                      : {}}
                />
                {cuenta.length > 0 && cuenta.length < 3 && (
                  <span style={{ fontSize: '0.78rem', color: 'var(--danger, #e74c3c)', marginTop: '0.25rem', display: 'block' }}>
                    Mínimo 3 caracteres ({3 - cuenta.length} restantes)
                  </span>
                )}
                {cuenta.length >= 3 && (
                  <span style={{ fontSize: '0.78rem', color: 'var(--success, #2ecc71)', marginTop: '0.25rem', display: 'block' }}>
                    ✓ Cuenta válida
                  </span>
                )}
              </div>
